...
class PessoaFisica(Cliente):
    def __init__(self, nome, data_nascimento, cpf, endereco):
        super().__init__(endereco) #trazendo atributo da classe pai
        self.nome = nome
...
class Conta:
    ...
    def sacar(self, valor):
        ...
        if excedeu_saldo:
            print("\nOperação não realizada! Seu saldo é insuficiente.")
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        ...
        if excedeu_limite:
            print("\nOperação não realizada! O valor solicitado excede o limite da conta.")

        elif excedeu_saques:
            print("\nOperação não realizada! Quantidade máxima de saqies excedida.")
    ...
    def depositar(self, valor):
        if valor > 0:
            self._saldo += valor
            print(f"\nDepósito realiado com sucesso! Seu novo saldo é de {self._saldo}.")
...
def criar_cliente(clientes):
    ...
    if not cliente:
        print("\nO CPF informado já está vinculado a um cliente.")
        return
